...
conn.sendall(str(neuronnum).encode())
time.sleep(1)
# set rng seed
seednum = 1000#int(input('Seed: '))
np.random.seed(seednum)
conn.sendall(str(seednum).encode())
...
for stim in initStim:
    conn.sendall(str(stim).encode())
    time.sleep(0.1)
    f = open('Responses','r')
    lineList = f.readlines()
    f.close()
    
    initResponse.append(int(lineList[-1]))
...
# Mutual Information
NC = Information_utility(probh,ph)
...
with open('Diagnostics_socket.csv','w') as d:
